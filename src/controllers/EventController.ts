...
export class EventController {
  ...
  static async deleteFile(req, res, next) {
    ...
    try {
      const pathname= event.files[req?.params.index]
  ...
  static async exportEvent(req, res, next) {
    ...
    try {
      ...
      if (req.user.role === "ADMIN") {
        console.log('admin')
        events = await Event.find({ user_id: req.query.userId })
      }
      if (req.user.role === "USER") {
        console.log('user')
        events = await Event.find({ user_id: req.user.user_id })
      }
